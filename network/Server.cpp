...
void server::update_player_state_and_send(query &query_, answer &answer_, sf::FloatRect &nextPositionCollider, int index){
        if (players[index].state == game_state["playing"]){
            players[index].state = query_.state;
        }
        ...
        answer_.partner_state = players[partner_index].state;
...
bool server::is_connection_correct(){
    std::unique_lock l(m);
    int state0 = players[0].state;
    int state1 = players[1].state;
    l.unlock();
    if (socket1.getRemoteAddress() == sf::IpAddress::None){
        if ((players[0].socket_number == 0 && players[0].state == game_state["playing"]) || players[1].socket_number == 0 && players[1].state == game_state["playing"]){
            return false;
        }
    }

    if (socket2.getRemoteAddress() == sf::IpAddress::None){
        if ((players[0].socket_number == 1 && players[0].state == game_state["playing"]) || players[1].socket_number == 1 && players[1].state == game_state["playing"]){
            return false;
        }
    }

    if (socket1.getRemoteAddress() == sf::IpAddress::None && socket2.getRemoteAddress() == sf::IpAddress::None){
        return false;
    }
    return true;
}
...
void server::updateScene(int num){
    ...
    while(is_connected){
        ...
        l.unlock();
        is_connected = is_connection_correct();
        if (!is_connected){
            socket1.disconnect();
            socket2.disconnect();
