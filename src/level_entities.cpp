...
void level_entities::init(std::string description_file) {
    ...
    try {
        ...
        );  // передали проект и забрали оттуда уровень  // достали папку где
            // лежит проект, чтобы потом там искать текстуры
...
void level_entities::check_coin_collision(
    sf::FloatRect &nextPositionCollider,
    answer &answer_
) {
    for (int i = 0; i < coins.size(); ++i) {
        if (nextPositionCollider.intersects(
                coins[i].coin_sprite.getGlobalBounds()
            )) {
            coins[i].disable();
...
void level_entities::check_enemy_collision(
    sf::FloatRect &nextPositionCollider,
    ...
    for (int i = 0; i < enemies.size(); ++i) {
        if (nextPositionCollider.intersects(
                enemies[i].enemySprite.getGlobalBounds()
            ) &&
            enemies[i].get_state() == EnemyState::active) {
            if (nextPositionCollider.top + nextPositionCollider.height - 4 <=
                    enemies[i].enemySprite.getPosition().y &&
                movement.y > 0) {
                enemies[i].disable();
...
void level_entities::check_collider_collision(
    sf::FloatRect &nextPositionCollider,
    sf::Vector2f &movement,
    answer &answer_
) {
